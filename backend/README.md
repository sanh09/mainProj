...
2. 텍스트 정제 / 조항 분리
3. 위험 조항 필터링 (LLM 병렬, `RISK_ASSESSOR_WORKERS`)
4. 판례/법령 수집 (병렬, `REFERENCE_FETCH_WORKERS`)
5. 임베딩 생성 + 유사도 매칭
6. 위험 유형 매핑
...
## Performance tuning (2026-02-04)
- `RISK_ASSESSOR_WORKERS`: risk LLM parallel workers (default 4)
- `REFERENCE_FETCH_WORKERS`: precedent/law fetch parallel workers (default 4)
- `DEBATE_MAX_ROUNDS`: debate max rounds (default 2)

Notes:
- `pipeline.analyze()` default now skips debate (`run_debate=False`).
- Enable debate by calling `pipeline.analyze(file_path, run_debate=True)`.



## Pinecone Migration (2026-02-13)

`precedent_store.py` / `law_store.py` now use Pinecone instead of PostgreSQL pgvector for reference data cache and vector search.

### Required env vars
- `PINECONE_API_KEY`: Pinecone API key

### Optional env vars
- `PINECONE_INDEX_NAME`: default `contract-rag`
- `PINECONE_CLOUD`: default `aws`
- `PINECONE_REGION`: default `us-east-1`
- `PINECONE_NAMESPACE_PRECEDENT`: default `precedents`
- `PINECONE_NAMESPACE_LAW`: default `laws`
- `EMBEDDING_DIM`: must match your embedding model dimension (default `1536`)

### Behavior notes
- If `PINECONE_API_KEY` is missing, store/search functions return empty results (pipeline falls back to Open API fetch path).
- Keyword cache lookup (`search_precedents/search_laws`) now embeds the keyword query and runs vector search.
- `search_*_by_vector` is backed by Pinecone query.
